...
public class Graph {
	...
	/**
	 * Add an edge to the graph. An edge between UserNodes and RestaurantNodes if
	 * the user reviewed the restaurant. The parameter values are used to create the
	 * UserNode and the RestaurantNode. The HashSets are used to ensure that
	...
	/**
	 * This is a helper method used to add nodes to the adjacency list.
	 * 
	 * @param node1
	 * @param node2
	 */
	...
	/**
	 * This method gets restaurant recommendations, given a minimum number of
	 * co-reviewers.
	 * 
	 * @param userID
	 * @param numCoReviewers
	 * @return List<RestaurantWrapper>
	 */
	public List<RestaurantWrapper> getRecommendationsWithMinCoReviewers(String userID, int numCoReviewers) {
		...
		List<RestaurantWrapper> recommendationsList = findRecommendationsMinCoReviewerHelper(recommendations.root,
				numCoReviewers);
		return recommendationsList;
	}

	/**
	 * This is a recursive helper method used to find recommendations given a
	 * minimum number of co-reviewers.
	 * 
	 * @param root
	 * @param numCoReviewers
	 * @return List<RestaurantWrapper>
	...
	/**
	 * A breadth first search populates the recommendationsBST instance variable in
	 * a given UserNode (identified by userId). The BST is populated with
	 * restaurants 3 hops away above the parameter rating. If a UserNode with userId
	 * does not exist in the graph, then return an empty ArrayList.
	 * 
	 * @param userId
				...
				if (hops == 2 && nextNode.isUser()) {
					...
					for (INode node : neighbours) {
						if (!visitedNodes.contains(node)) {
							...
							// Store each co-reviewers rating of the recommended restaurant
	...
	/**
	 * Gets the map of ids to UserNodes.
	 * 
	 * @return HashMap
	 */
	...
	/**
	 * Gets a RestaurantNode given an id
	 * 
	 * @param restaurantId
	...
	/**
	 * Gets a map of ids to RestaurantNodes
	 * 
	 * @return HashMap
	 */
